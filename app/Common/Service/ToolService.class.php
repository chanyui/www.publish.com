...
class ToolService{
     ...
     * @return bool
     */
    public static function setResubmit($key='', $data='', $expire=58){
        if(!$key) return false;
        $key = self::encodeRedisKey('resubmit', $key, true);
        return RedisLService::getInstance()->set($key, $data, $expire);
    }
     ...
     * @return string
     */
    public static function encodeRedisKey($type, $key, $ipCheck=false){
        $m = MODULE_NAME;
        $c = CONTROLLER_NAME;
        $a = ACTION_NAME;
        if($ipCheck){
     ...
     * @return int
     */
    public static function checkResubmitIsExpired($key){
        $key = self::encodeRedisKey('resubmit', $key, true);
        return RedisLService::getInstance()->exists($key);
    }
